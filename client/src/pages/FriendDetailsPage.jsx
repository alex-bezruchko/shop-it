...
export default function FriendDetailPage({listLoading}) {
  ...
  const [friend, setFriend] = useState({ user: { favoritePlaces: [], name: '' }, lists: [] });
            ...
            {friend.user.favoritePlaces.length !== 0 ? (
              friend.user.favoritePlaces.map(place => (
                <div key={place.place_id} className="w-full flex my-5 flex-col items-center justify-between bg-white rounded-lg shadow-lg p-3 mb-4 border border-[1.5px] border-primaryBlue">
                  ...
                  <svg onClick={() => togglePlace(place)} xmlns="http://www.w3.org/2000/svg"  fill={isPlaceFavorite(place) ? 'rgb(247, 160, 114)' : 'none'}  viewBox="0 0 24 24" strokeWidth="1.5" stroke={isPlaceFavorite(place) ? 'none' : 'rgb(247, 160, 114)'} className="min-w-[40px] h-[40px] text-primaryBlue">
              ...
              <h3 className='text-center nunito text-xl my-6'>Your friend has no places yet.</h3>
            )}
          </div>
        </div>
      )}
                      ...
                      {currentList.products?.map(product => (
                          <li key={product.product._id} className="flex w-full justify-between">
                              <div className="flex items-center w-full justify-between bg-white rounded-lg shadow-lg p-0 mb-4 border border-2 border-primaryBlue">
                                  ...
                                  <div className="flex items-center">
                                      <img 
                                          src={product.product.photo === 'placeholder.png' ? placeholderImg : `${product.product.photo}?fit=crop&h=175&w=175&crop=entropy&q=80`}
