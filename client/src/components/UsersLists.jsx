                    ...
                    {currentLists.length !== 0 && (
                        <div className="w-full">
                            <div className="tabs flex justify-around">
                                ...
                                <button onClick={() => handleTab('completed')} className={activeTab === 'completed' ? 'text-center nunito text-xl p-2 w-1/2 rounded sm:text-3xl lists bg-primaryGreen flex justify-around items-center text-white' : 'text-center text-black nunito text-xl p-2 w-1/2 rounded sm:text-3xl lists flex justify-around items-center'}>
                                    <span>Completed</span>
                                    {activeTab === 'completed' ? (
                            ...
                            <ul className="mt-8">
                                {filteredList.map(list => (
                                    <div key={list._id} className="flex w-full justify-between">
                                        <div onClick={() => viewList(list._id)} className={`flex w-full items-center justify-between bg-white rounded-lg shadow-lg p-4 mb-4 border border-[1.5px] ${list.completed ? 'border-primaryGreen' : 'border-primaryOrange'}`}>
                                ...
                                {filteredList.length === 0 && (
                                    <h2 className="flex text-center text-xl nunito justify-center mt-2">
                                        No lists found. Create one here: 
                                        
                                        <Link to={'/account/new'}>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="#6CB462" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="h-full ml-2 text-white">
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m3.75 9v6m3-3H9m1.5-12H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" className="h-full"/>
                                            </svg>
                                        </Link>
                                    </h2>
                                )}
