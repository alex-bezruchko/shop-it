        ...
        {products.products.map(product => (
            <div key={product._id} className="flex w-full justify-between">
              <div className={`flex items-center w-full justify-between bg-white rounded-lg shadow-lg p-0 md:p-0 mb-4 border border-2 ${isProductAdded(product) ? 'border-primaryGray' : 'border-primaryGreen'}`}>
                ...
                {product.photo && (
                    <img src={product.photo} alt="Product Photo" className="cursor-pointer mr-0 max-h-[95px] min-h-[95px] min-w-[95px] max-w-[95px] sm:max-h-[100%] sm:min-h-[100%] sm:min-w-[100%] sm:max-w-[100%] rounded-r-md"/>)
                }
              ...
              <button onClick={() => handleAddToList(product)} className={`text-primaryGreen ml-2 focus:outline-none ${isProductAdded(product) ? 'text-primaryGray' : ''}`}>
