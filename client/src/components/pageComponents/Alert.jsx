...
const AlertContainer = ({ alert, removeAlert }) => {
  ...
  return (
    <div className='flex justify-center'>
      {message && alertType && (
        <div className={`w-full text-center md:w-2/3  flex justify-between sm:justify-between mt-8 p-3 rounded border border-[1.5px] border-${alertType} ${alertType === 'primaryGreen' ? 'bg-lightGreen' : alertType === 'primaryOrange' ? 'bg-lightOrange' : 'bg-lightRed'}` }>
          <span>{message}</span>
